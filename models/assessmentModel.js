...
const AssessmentModel = {
  ...
  return rows[0];
    ...
    async updateById(id,data){
        ...
        created_by_user_id}=data;

        const{ rows } = await pool.query(
            `UPDATE assessments
            SET
            title = COALESCE($1,title),
            descripton=COALESCE($2,description),
            duration_minutes =COALESCE($3,duration_minutes),
            created_by_user_id=COALESCE($4,created_by_user_id)
            WHERE assessment_id =$5
            RETURNING assessment_id,title,description,duration_minutes,
            created_by_user_id`
            [title??null,description??null,duration_minutes??null,
            created_by_user_id??null,id ]
        );
        return rows[0]|| null;
    },
    //Delete operation
